...
export default {
  ...
  watch: {
    childWidth: {
      handler(val) {
        console.log("watch-childWidth", val);
        // this.ezopenInit();
        // this.videoPlayer[0].reSize(this.childWidth, this.childHeight);
      },
      // frameList: {
      //   handler(val) {
      //     console.error("watch-id", val);
      //   },
      // },
  ...
  methods: {
    ...
    // 获取播放地址
    fetchLiveAddress(deviceSerial, channelNo, i) {
      getLiveAddress(deviceSerial, channelNo).then((response) => {
        console.log("fetchLiveAddress", response);
        console.log(deviceSerial + channelNo + "-window", channelNo, i);
        this.ezopenInit(
          deviceSerial + "-" + channelNo + "-window",
          response.msg,
          i
        );
      });
    ...
    clickMenu(item, channelNo) {
      ...
      for (let i = 0; i < this.screenIndex; i++) {
        if (this.frameList[i].id == "-") {
          id = item + "-" + channelNo;
          this.$set(this.frameList, i, { id: id });
          console.log("this.frameList", this.frameList);

          // document.getElementById(this.frameList[i].id)
          //   .firstChild.setAttribute("id", this.frameList[i].id + "-window");
          // setTimeout(() => {
          ...
          this.$nextTick(() => {
            document
              .getElementById(id)
              .firstChild.setAttribute("id", id + "-window");
            const sendId = item + "-" + channelNo;
            console.log("sendId", sendId);
            this.fetchLiveAddress(item, Number(channelNo), i);
          });
          return;
          // return
        }
    ...
    closeVideo(index) {
      ...
      const closeIndex = document.getElementById(
        `${this.frameList[index].id}-window-wrap`
      );
    ...
    changeNum(index) {
      ...
      // this.fetchLiveAddress(this.deviceSerial, this.id);
      // this.ezopenInit();
